...
class Text2LatentDataset(Dataset):
    ...
    def _phonemize_with_espeak(self, mask):
        ...
        try:
            ...
            raise ImportError("phonemizer is required for non-Hebrew TTS data. Install it with: pip install phonemizer")
...
def collate_text2latent(batch):
    ...
    wavs_padded = torch.stack(wavs_padded).unsqueeze(1)
    ...
    ref_padded = torch.stack(ref_padded).unsqueeze(1)
    ...
    for t in texts:
        ...
        txt_padded.append(F.pad(t, (0, pad), value=0))
        mask = torch.zeros(max_txt, dtype=torch.float32)
        mask[:t.shape[0]] = 1.0
        txt_masks.append(mask)
    txt_padded = torch.stack(txt_padded)
    txt_masks = torch.stack(txt_masks).unsqueeze(1)
