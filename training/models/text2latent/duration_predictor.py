...
class DPReferenceEncoder(nn.Module):
    ...
    def forward(self, z_ref: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        B = z_ref.shape[0]
...
class DPTextEncoder(nn.Module):
    def __init__(self, vocab_size: int = 37, d_model: int = 64):
        super().__init__()
    ...
    def forward(
        ...
        # Prepend learnable sentence token
        ...
        # Pool to sentence embedding via the first (sentence) token
        first_token = x[:, :, :1]
...
class DurationEstimator(nn.Module):
    def __init__(self, text_dim: int = 64, style_dim: int = 128):
        super().__init__()
...
class TTSDurationModel(nn.Module):
    ...
    def forward(
        ...
        return self.predictor(text_emb, style_emb, return_log=return_log)
