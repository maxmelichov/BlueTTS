"""
Training script for the Text-to-Latent model.

References:
  SuperTonic TTS:           https://arxiv.org/abs/2503.23108
  Self-Purifying FM (SPFM): https://arxiv.org/abs/2512.17293
  Flow Matching:            https://arxiv.org/abs/2509.19091
  LARoPE:                   https://arxiv.org/abs/2509.11084
"""
import os
import sys
import glob
...
class UncondParams(nn.Module):
    """Learnable unconditional tokens for CFG (https://arxiv.org/abs/2503.23108 Sec 3.2.4)."""
...
def train(
    ...
    dp_checkpoint="checkpoints/duration_predictor/duration_predictor_final.pt",
    ref_wav_path="",
    cross_ref_prob=0.5,
):
    ...
    if ckpts:
        ...
        if 'global_step' in checkpoint:
            ...
            if finetune:
                ...
                print(f"Finetune mode: global_step={global_step}, spfm_start_override={spfm_start_override}")
    ...
    # lr schedule from https://arxiv.org/abs/2503.23108 Sec 4.2: halved at 300k and 600k steps
    ...
    while global_step < max_steps:
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            _, _, T_txt = h_text.shape

            # Self-Purifying Flow Matching (https://arxiv.org/abs/2512.17293)
            ...
            if global_step >= spfm_start and global_step <= end_spfm:
                with torch.no_grad():
                    ...
                    _, D_text_spfm, _ = h_text_spfm.shape
            ...
            # Batch expansion (https://arxiv.org/abs/2503.23108 Sec 3.2.2 / Algorithm 1)
            ...
            with torch.no_grad():
                ...
                # Flow matching interpolation: x_t = (1-(1-σ)t)x_0 + t·z_1
                x_t = (1 - (1 - sigma_min) * t_broad) * x_0 + t_broad * z_1_exp
                v_target = z_1_exp - (1 - sigma_min) * x_0

            # CFG dropout (https://arxiv.org/abs/2503.23108 Sec 3.2.4)
            ...
            if global_step % 1000 == 0 and rank == 0:
                ...
                try:
                    wav_preds = sample_audio(
                        ...
                        val_text_ids, val_text_masks,
                        val_z_ref, val_ref_enc_mask,
                        u_text, u_ref, u_keys,
                        mean, std,
                    ...
                    hebrew_sentences = [
                        "ʃalˈom moʃˈe kˈaχa niʃmˈa hamˈodel heχadˈaʃ mˈa daʔtχˈa ? lifʔamˈim tsaʁˈiχ baχajˈim lelatˈeʃ ʁaʔjˈon ʃˈuv vaʃˈuv ʔˈad ʃehˈu matslˈiaχ"
                    ]
                    ...
                    try:
                        ...
                        _en_backend = EspeakBackend('en-us', preserve_punctuation=True, with_stress=True, language_switch='remove-flags')
                        english_sentences = [normalize_text(s, lang='en') for s in _en_backend.phonemize(english_sentences_raw, separator=_sep)]
                    ...
                    try:
                        from phonemizer.backend import EspeakBackend
                        from phonemizer.separator import Separator
                        _de_backend = EspeakBackend('de', preserve_punctuation=True, with_stress=True, language_switch='remove-flags')
                        german_sentences = [normalize_text(s, lang='de') for s in _de_backend.phonemize(german_sentences_raw, separator=Separator(phone='', word=' ', syllable=''))]
                    except Exception as _e:
                    ...
                    try:
                        from phonemizer.backend import EspeakBackend
                        from phonemizer.separator import Separator
                        _it_backend = EspeakBackend('it', preserve_punctuation=True, with_stress=True, language_switch='remove-flags')
                        italian_sentences = [normalize_text(s, lang='it') for s in _it_backend.phonemize(italian_sentences_raw, separator=Separator(phone='', word=' ', syllable=''))]
                    except Exception as _e:
                    ...
                    try:
                        from phonemizer.backend import EspeakBackend
                        from phonemizer.separator import Separator
                        _es_backend = EspeakBackend('es', preserve_punctuation=True, with_stress=True, language_switch='remove-flags')
                        spanish_sentences = [normalize_text(s, lang='es') for s in _es_backend.phonemize(spanish_sentences_raw, separator=Separator(phone='', word=' ', syllable=''))]
                    except Exception as _e:
                    ...
                    def run_inference_for_ref(ref_wav_torch, suffix, sentences, lang, label):
                        ...
                        with torch.no_grad():
                            ...
                            ref_z, ref_mask = build_reference_only(ref_z_norm, valid_z_len_ref, device)
                        for i, text in enumerate(sentences):
                            ...
                            wav_out = sample_audio(
                                vf_infer, te_infer, re_infer, ae_decoder,
                                txt_ids, txt_mask, ref_z, ref_mask,
                                u_text, u_ref, u_keys, mean, std,
                                duration_predictor=dp_model,
                                steps=16, cfg_scale=3.0, device=device,
                    ...
                    if val_batch is not None:
                        ...
                        for lang, sentences, label in [
                            ...
                            for i, text in enumerate(sentences):
                                ...
                                wav_out = sample_audio(
                                    vf_infer, te_infer, re_infer, ae_decoder,
                                    txt_ids, txt_mask, ref_z_val, ref_mask_val,
                                    u_text, u_ref, u_keys, mean, std,
                                    duration_predictor=dp_model,
                                    steps=16, cfg_scale=3.0, device=device,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints/text2latent",
    ...
    parser.add_argument("--metadata_path", type=str, default="generated_audio/combined_dataset_cleaned_real_data.csv",
                        help="Path to dataset metadata CSV")
    parser.add_argument("--dp_checkpoint", type=str, default="checkpoints/duration_predictor/duration_predictor_final.pt",
                        help="Path to duration predictor checkpoint")
    parser.add_argument("--ref_wav_path", type=str, default="",
                        help="Path to reference wav for inference during training (optional)")
    parser.add_argument("--cross_ref_prob", type=float, default=0.5,
                        help="Probability of using cross-speaker reference (default: 0.5)")
