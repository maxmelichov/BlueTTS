...
def build_reference_from_latents(z_1, valid_z_len, z_ref_input, valid_z_ref_len, is_self_ref, device, chunk_compress_factor=6):
    ...
    # Derive compressed_rate: 44100 / 512 / factor ~= 14.35 Hz
    sr = 44100
    hop = 512
...
def sample_audio(
    ...
    if duration_predictor is None:
        ...
        print("[DBG] T_lat    =", T_lat.detach().cpu().numpy())
...
def train(
    ...
    while global_step < max_steps:
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            # Debug: Verify Mask Overlap (Once)
            if global_step == 0:
                 pass
            ...
            # We need both versions expanded
            # text_masks_cond_exp = text_masks_cond.repeat_interleave(Ke, dim=0) # Unused
            text_masks_base_exp = text_masks.repeat_interleave(Ke, dim=0) # Padding only
            ...
            if global_step % 1000 == 0 and rank == 0:
                ...
                try:
                    ...
                    # Function to run inference for a list of sentences
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints/text2latent",
                        help="Directory to save checkpoints (default: checkpoints/text2latent)")
